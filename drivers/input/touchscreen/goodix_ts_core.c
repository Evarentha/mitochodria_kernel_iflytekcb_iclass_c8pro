...
/* ts_suspend - for vendor init.leddrv.rc compatibility */
static ssize_t goodix_ts_suspend_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct goodix_ts_core *cd = dev_get_drvdata(dev);
	return snprintf(buf, PAGE_SIZE, "%d\n",
		cd ? atomic_read(&cd->suspended) : 0);
}
static ssize_t goodix_ts_suspend_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	return count;
}
static DEVICE_ATTR(ts_suspend, S_IRUGO | S_IWUSR | S_IWGRP,
		   goodix_ts_suspend_show, goodix_ts_suspend_store);
