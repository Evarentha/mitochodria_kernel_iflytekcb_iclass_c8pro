...
static void sprd_first_keyboard_connect_work(struct work_struct *work)
{
	...
	if (!gpio_is_valid(keyboard_irq) && !gpio_is_valid(hall_pin))
		return;

	if (gpio_is_valid(keyboard_irq) && gpio_get_value(keyboard_irq) == 0) {
	...
	if (gpio_is_valid(keyboard_irq)) {
		irq_num = gpio_to_irq(keyboard_irq);
		ret = request_irq(irq_num, sprd_keyboard_isr,
			IRQF_TRIGGER_LOW | IRQF_ONESHOT, "sprd_keybd", NULL);
		if (ret)
			pr_err("Failed to request keyboard IRQ\n");
		enable_irq_wake(irq_num);
		disable_irq(irq_num);
	}
	if (gpio_is_valid(hall_pin)) {
		hall_irq_num = gpio_to_irq(hall_pin);
		ret = request_irq(hall_irq_num, sprd_hall_isr,
			IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING | IRQF_ONESHOT,
			"sprd_hall", NULL);
		if (ret)
			pr_err("Failed to request hall IRQ\n");
	}
}
